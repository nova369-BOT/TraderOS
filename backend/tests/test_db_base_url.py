...
def test_get_database_url_syncs_with_settings(monkeypatch) -> None:
    ...
    if sys.platform.startswith("win"):
        test_url = f"sqlite:///{tmp}/tos_test/test.db"
        expected_url = f"sqlite+aiosqlite:///{tmp}/tos_test/test.db"
    else:
        test_url = f"sqlite:///{tmp}/tos_test/test.db"
        expected_url = f"sqlite+aiosqlite:///{tmp}/tos_test/test.db"

    monkeypatch.setenv("TRADEOS_SQLITE_URL", test_url)
...
def test_get_sync_database_url_normalizes_aiosqlite(monkeypatch) -> None:
    monkeypatch.setenv("DATABASE_URL", "sqlite+aiosqlite:////tmp/tradeos.db")
    get_settings.cache_clear()

    url = get_sync_database_url()
    assert url == "sqlite:////tmp/tradeos.db"


def test_sqlite_file_from_url_supports_async_sqlite_urls() -> None:
    path = sqlite_file_from_url("sqlite+aiosqlite:////tmp/tradeos.db")
    assert path == Path("/tmp/tradeos.db").resolve()


def test_get_database_url_default_is_workspace_local(monkeypatch) -> None:
    monkeypatch.delenv("DATABASE_URL", raising=False)
    monkeypatch.delenv("TRADEOS_SQLITE_URL", raising=False)
    monkeypatch.delenv("OPENSCREENS_SQLITE_URL", raising=False)
    monkeypatch.delenv("TRADE_SCREENS_SQLITE_URL", raising=False)
    get_settings.cache_clear()

    url = get_database_url()
    expected_path = (Path(__file__).resolve().parents[2] / "data" / "tradeos.db").resolve()
