...
// data.ts. No React state touches the paint path (refs + rAF).
...
export default function ChartPane({ theme }: Props) {
  ...
  const paint = () => {
    ...
    // keep easing the unfold until settled
  ...
  const onWheel = (e: React.WheelEvent) => {
    ...
    if (e.shiftKey) {
      ...
      // keep the candle under the cursor anchored
  ...
  return (
    <div ref={wrapRef} style={{ position: 'absolute', inset: 0 }}>
      <canvas
        ref={canvasRef}
        onWheel={onWheel}
        onDoubleClick={() => { viewRef.current = defaultView(); schedule(); }}
        onPointerDown={(e) => {
        ...
        onPointerLeave={() => { hoverRef.current = null; dragRef.current = null; schedule(); }}
