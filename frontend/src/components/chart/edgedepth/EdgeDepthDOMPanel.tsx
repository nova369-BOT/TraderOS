...
interface DOMData {
  bids: Level[];
  asks: Level[];
  buys?: Record<string, number>; // price -> qty
...
export function EdgeDepthDOMPanel({ symbol, provider = 'binance' }: { symbol: string; provider?: string }) {
  ...
  // RowModel cache — rebuild only when book ts/uid/acc_rev/center/scroll/group changed
  ...
  }, [symbol, provider, groupMult, showUsd, autoCenter]);
  ...
  // Live trades for BUYS/SELLS/DELTA accumulator
  useEffect(() => {
  ...
  if (!data) return <div className="p-2 text-[11px] text-[#b9b9b9] bg-[#2a2a2a] h-full">DOM loading {symbol}...</div>;

  return (
    <div className="h-full flex flex-col bg-[#2a2a2a] text-[#e8e8e8] font-mono text-[11px] select-none" onWheel={onWheel}>
      {/* Controls */}
      ...
      {/* Header BUYS/BIDS/PRICE/ASKS/SELLS/DELTA */}
      ...
      <div className="px-2 py-1 text-[9px] text-[#b9b9b9] border-t border-[#3a3a3a] bg-[#1c1c1c] shrink-0">
        Shift+wheel = group x1/x10/x100, wheel = scroll, auto-center {autoCenter?'ON':'OFF'} • SoA cache • double-buffered write/read swap once per frame
      </div>
    </div>
  );
}
