# ============================================================================
# TradeOS — one-command installer for Windows (PowerShell).
#
#   ./install.ps1                 # auto-detect: Docker if available, else local
#   $env:TOS_MODE="docker"; ./install.ps1
#   $env:TOS_MODE="local";  ./install.ps1
#
...
Write-Cyan "==> TradeOS installer (Windows)"
...
Initialize-EnvVar "BOOTSTRAP_ADMIN_EMAIL" "admin@tradeos.local"
...
# --- 3. Pick a run mode ----------------------------------------------------
$Mode = if ($env:TOS_MODE) { $env:TOS_MODE } else { "auto" }
if ($Mode -eq "auto") {
...
function Show-Credentials {
  Write-Host ""
  Write-Green "============================================================"
  Write-Green " TradeOS is ready  ->  http://localhost:$Port"
  Write-Green "------------------------------------------------------------"
