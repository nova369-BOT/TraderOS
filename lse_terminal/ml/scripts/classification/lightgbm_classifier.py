#!/usr/bin/env python3
"""
LightGBM Classifier - CPU/GPU Training Script
Fetches data via utils.py, computes selected features, trains LightGBM classifier.
"""
import sys, os
...
def run_model(df, args, feature_names):
    ...
    y = (close[horizon:] > close[:-horizon]).astype(int)
    X = X[:len(y)]
    ...
    try:
        ...
        try:
            ...
            if torch.cuda.is_available():
                gpu_params = {"device": "gpu", "gpu_use_dp": True}
                print("[GPU] LightGBM using CUDA acceleration")
        except ImportError:
            pass
        model = lgb.LGBMClassifier(n_estimators=args.n_estimators, max_depth=args.max_depth,
                                    learning_rate=args.learning_rate, verbose=-1, **gpu_params)
        model.fit(X_train, y_train)
        preds = model.predict(X_test)
        proba = model.predict_proba(X_test)[:, 1]
        # LightGBM feature_importances_ returns split counts by default (e.g. 234, 128).
        ...
        model = GradientBoostingClassifier(n_estimators=args.n_estimators, max_depth=min(args.max_depth, 3),
                                           learning_rate=args.learning_rate)
        model.fit(X_train, y_train)
        preds = model.predict(X_test)
        proba = model.predict_proba(X_test)[:, 1]
        # sklearn feature_importances_ sums to 1.0
        raw_imp = np.array(model.feature_importances_.tolist())
    ...
    return {
        "metrics": {"accuracy": acc, "precision": precision, "recall": recall, "f1_score": f1,
                    "test_samples": int(len(y_test)), "train_samples": int(len(y_train))},
        "feature_importance": dict(sorted(importance.items(), key=lambda x: x[1], reverse=True)[:10]),
    }


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="LightGBM Classifier")
    ...
    print(f"[INFO] Starting LightGBM Classifier...")
