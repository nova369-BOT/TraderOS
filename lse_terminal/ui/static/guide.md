...
- **Depth Heat controls.** The slider above the pane narrows or widens the cut-off window (where the gradient saturates). The ⚙ button — or right-click on the heat — opens the settings window: colour scheme (heat / greyscale, with "apply globally" remembered for the whole terminal), intensity and dimming, percentile or exact cut-offs with the resolved sizes shown, vertical smoothing (auto adapts to your price zoom), contrast and brightness, volume dots (minimum accountable size, size, transparency, and gradient / solid / pie drawings — pie splits each price-time cell by aggressor-side volume), the COB column set, an active-range override whose boundary lines mark the window on the ladder, auto-recentering on BBO or trades with a tolerance band, and the depth reset policy. Everything applies live and persists per instrument.
