...
- **Depth Heat (order flow liquidity).** The 🔥 depth button in the top-right corner of a chart pane flips that pane into the Depth Heat view — in the single-chart layout AND in every pane of a multi-chart layout (the choice is remembered per pane; the pane's own chart ⇄ button flips back): a time × price field where resting limit-order size at each price is painted as colour, brighter toward the solid end of the map, scrolling right in real time exactly like the chart beside it. The HEAT/FOOTPRINT switch in the pane header swaps the liquidity field for a classic footprint: executed volume split sell×buy per price zone and time bucket (bucket width follows your time zoom), zones imbalanced 3:1 or more tinted, and a per-bucket delta and total volume along the bottom. The footprint is built only from side-stamped prints; a source that carries none shows a blank with the reason instead of invented numbers. The ladder shows the same book as numbers — by default FUSED into the price-axis gutter (price + size bar + size per level, Bookmap layout, active-range boundary lines included); the separate COB panel (per-level size bars, running cumulative, the spread between the BBO rows) remains an option in settings, pixel-aligned with the heat's price axis. The header's T&S button opens a time & sales drawer listing every executed print with a min-size filter and ALL/BUY/SELL. Wheel zooms time, shift+wheel zooms price, drag pans, double-click recentres; the pane's crosshair time stays synced with the other panes.
- **Depth Heat data sources.** Real depth only where real depth exists, in this order: vault MBO futures for keys whose plan carries the order-by-order capture (true L3 rebuilt into the book); public crypto L2 (BTC/USD, ETH/USD and six more, Coinbase primary with Kraken fallback, no key needed — exchanges expose no L2 history, so these symbols heat from the live feed and the window shown is exactly what exists); and the terminal's own recordings. DEMO symbols run a clearly labelled synthetic feed for exploring the pane. Where no source carries depth for a symbol, the pane says so with the reason instead of inventing data.
- **Depth Heat controls.** The slider above the pane narrows or widens the cut-off window (where the gradient saturates). The ⚙ button — or right-click on the heat — opens the settings window: colour ramp (DEEPDOM side-aware ember/water, BOOKMAP blue-water-with-hot-walls, classic heat, greyscale — with "apply globally" remembered for the whole terminal), perceptual intensity γ, wall glow, smooth columns, intensity and dimming, percentile or exact cut-offs with the resolved sizes shown, vertical smoothing (auto adapts to your price zoom), contrast and brightness, overlays (stepped bid/ask price path, trade-derived candles over the heat, big-trade rings + size tags at a configurable multiple of the rolling median size, and the collapsible bottom context stack — buy/sell-split volume histogram plus a CVD line sharing the time axis — with Imb/Cvd gauges at the top left), volume dots (minimum accountable size, size, transparency, and sphere / solid / pie drawings — pie splits each price-time cell by aggressor-side volume), the COB column set, an active-range override whose boundary lines mark the window on the ladder, auto-recentering on BBO or trades with a tolerance band, and the depth reset policy. The right edge always carries a price axis with tick grid, BBO chips and a side-coloured last-price box. Everything applies live and persists per instrument.
- **Depth Heat recording.** ● REC above the pane captures the symbol's live depth and trades into a parquet session in workspace/MY DATA; the 🗂 sessions button lists them with size, duration and rows, loads a recording back into the pane (the live feed returns with ✕ on the SESSION LOADED chip) and deletes old ones. Recorded sessions are what gives live-only symbols their history.
