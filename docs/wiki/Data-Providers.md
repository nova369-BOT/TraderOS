...
TradeOS uses a **multi-provider waterfall architecture** — each data request tries the highest-quality provider first, and falls back automatically on failure. This ensures resilience against rate limits, scraping breakage, and API outages.
...
- **Cache warming:** Set `TRADEOS_PREFETCH_ENABLED=1` to enable background prefetch for configured symbols on startup.
