...
**Goal:** Build the backend of TradeOS's agent framework — a multi-provider LLM abstraction (OpenRouter primary), a typed tool registry wrapping existing services, a native tool-calling orchestrator loop, and an SSE API — delivering read-only "determine stocks" runs end-to-end.
...
def test_agent_defaults(monkeypatch):
    for var in [
        "TRADEOS_OPENROUTER_API_KEY", "OPENROUTER_API_KEY",
        "TRADEOS_AGENT_PROVIDER", "TRADEOS_AGENT_MODEL",
    ]:
...
def test_agent_env_override(monkeypatch):
    ...
    monkeypatch.setenv("TRADEOS_AGENT_MODEL", "openai/gpt-4o")
    s = _fresh_settings()
...
SYSTEM_PROMPT = (
    "You are the TradeOS financial analysis agent. Help the user analyze and "
