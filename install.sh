#!/usr/bin/env bash
# ============================================================================
# TradeOS — one-command installer.
#
#   ./install.sh                # auto-detect: Docker if available, else local
#   TOS_MODE=docker ./install.sh
#   TOS_MODE=local  ./install.sh
#
...
# Sets: TOS_OS  (macos|linux|wsl|windows)
...
detect_os() {
  ...
  case "$uname_s" in
    ...
    Linux)
      ...
      ;;
    MINGW*|MSYS*|CYGWIN*) TOS_OS="windows" ;;
    *) TOS_OS="unknown" ;;
  esac
...
cyan "==> TradeOS installer"

# --- 0. Detect host OS and adapt ------------------------------------------
detect_os
green "    detected OS: ${TOS_OS}  (python: ${PY_BIN:-not found})"
if [ "$TOS_OS" = "windows" ]; then
...
ensure_var BOOTSTRAP_ADMIN_EMAIL "admin@tradeos.local"
...
if [ "$MODE" = "auto" ]; then
  ...
  if command -v docker >/dev/null 2>&1 && docker compose version >/dev/null 2>&1 && docker info >/dev/null 2>&1; then
    ...
    if command -v docker >/dev/null 2>&1 && ! docker info >/dev/null 2>&1; then
      ...
      yellow "    (start Docker Desktop and re-run, or force it with: TOS_MODE=docker ./install.sh)"
    fi
    MODE="local"
  fi
fi
...
print_credentials() {
  echo
  green "============================================================"
  green " TradeOS is ready  ->  http://localhost:${PORT}"
  green "------------------------------------------------------------"
  green "  Log in with:"
